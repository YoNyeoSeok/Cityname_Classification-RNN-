...
for iter in range(1, args.n_iters + 1):
    dataBatch = trainDataBatch.getBatch(batch_size)
    categories, category_idxs, lines = trainDataBatch.dataSplit(dataBatch)
    category_tensor, line_tensor = data2tensor.batchDataToPackSequence(category_idxs, lines)
...
#    category, line, category_tensor, line_tensor = randomTrainingExample()
    ...
    val_dataBatch = validDataBatch.getBatch(batch_size)
    val_categories, val_category_idxs, val_lines = validDataBatch.dataSplit(val_dataBatch)
    val_category_tensor, val_line_tensor = data2tensor.batchDataToPackSequence(val_category_idxs, val_lines)
